...
class DecodingInitStateEncoder(nn.Module):
    ...
    def forward(self, c_ids, zq=None, za=None):
        ...
        assert (zq is not None and za is not None) or \
            (zq is None and za is None)
        ...
        c_zq_concat = torch.cat([c_h, zq], dim=-1)
        q_init_c = self.q_c_init_state_linear(c_zq_concat)
        q_init_h = self.q_h_init_state_linear(c_zq_concat)
