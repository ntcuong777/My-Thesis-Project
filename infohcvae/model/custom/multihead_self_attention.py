...
class ScaledDotProductAttention(nn.Module):
    ...
    def forward(self, query: Tensor, key: Tensor, value: Tensor, mask: Optional[Tensor] = None) -> Tuple[Tensor, Tensor]:
        ...
        if mask is not None:
            mask_ = mask
            if not (mask.dtype in [torch.bool]):
                mask_ = mask > 0
            score.masked_fill_(mask_.view(score.size()), -float('Inf'))
...
class CombinedMultiHeadLocationAttention(nn.Module):
    r"""
    Customizing Attention
    WARNINGS: Not a verified mechanism
    ...
    def __init__(self, hidden_dim: int, num_heads: int = 4, conv_out_channel: int = 10) -> None:
        super(CombinedMultiHeadLocationAttention, self).__init__()
        self.hidden_dim = hidden_dim
        self.num_heads = num_heads
        self.dim = int(hidden_dim / num_heads)
