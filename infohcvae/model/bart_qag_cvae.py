...
class BartQAGConditionalVae(pl.LightningModule):
    ...
    def add_model_specific_args(parent_parser):
        ...
        parser.add_argument('--lambda_mmd_q', type=float, default=0.5)
        parser.add_argument('--lambda_mmd_a', type=float, default=0.5)
    ...
    def build_za_past(self, za):
        projection = self.za_memory_projection(za, dim=-1)
        cross_attn = projection.reshape(
            self.config.num_decoder_layers,
            projection.shape[0],
            self.config.num_heads,
