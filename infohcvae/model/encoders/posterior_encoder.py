...
class PosteriorEncoder(nn.Module):
    def __init__(self, embedding, d_model, lstm_enc_nhidden, lstm_enc_nlayers,
        ...
        self.encoder = CustomLSTM(input_size=d_model, hidden_size=lstm_enc_nhidden,
                                  ...
                                  bidirectional=True)
        self.question_self_attention = GatedAttention(hidden_size=lstm_enc_nhidden * 2)
        self.question_luong_attention = LuongAttention(lstm_enc_nhidden * 2, lstm_enc_nhidden * 2)
        ...
        self.context_answer_self_attention = GatedAttention(hidden_size=lstm_enc_nhidden * 2)
        self.context_answer_luong_attention = LuongAttention(lstm_enc_nhidden * 2, lstm_enc_nhidden * 2)
    ...
    def forward(self, c_ids, q_ids, a_mask):
        ...
        q_hidden_states = self.question_self_attention(q_hidden_states, attention_mask=q_mask)
        ...
        q_h = self.question_luong_attention(q_h.unsqueeze(1), q_hidden_states, mask).squeeze(1)
        ...
        c_a_hidden_states = self.context_answer_self_attention(c_a_hidden_states, attention_mask=c_mask)
        c_a_h = c_a_state[0].view(self.nlayers, 2, -1, self.nhidden)[-1]
        c_a_h = c_a_h.transpose(0, 1).contiguous().view(-1, 2 * self.nhidden)
        # the final forward and reverse hidden states should attend to the whole sentence
        mask = c_mask.unsqueeze(1)
        c_a_h = self.context_answer_luong_attention(c_a_h.unsqueeze(1), c_a_hidden_states, mask).squeeze(1)
