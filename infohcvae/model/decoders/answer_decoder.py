...
class AnswerDecoder(nn.Module):
    ...
    def forward(self, c_ids, za, zq):
        ...
        for layer in self.answer_decoder:
            dec_hs = layer(dec_hs, c_lengths, c_mask, q_init_state=q_init_state)
    ...
    def generate(self, c_ids, za=None, zq=None, start_logits=None, end_logits=None):
        ...
        assert (zq is None and zq is None) or (za is not None and zq is not None), \
            "`za` and `zq` must be both provided or both empty"
        assert (start_logits is None and za is not None) or \
               (start_logits is not None and za is None), \
            "logits or latent codes must be provided while cannot providing both"
