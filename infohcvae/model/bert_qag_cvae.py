...
class BertQAGConditionalVae(pl.LightningModule):
    def __init__(self, args):
        ...
        self.a_discriminator = AnswerDiscriminator(
            embedding, d_model, encoder_nhidden, num_layers=1, dropout=encoder_dropout, pad_token_id=self.pad_token_id)

        """ Loss computation """
        self.q_rec_criterion = nn.CrossEntropyLoss(ignore_index=self.pad_token_id)
        self.a_rec_criterion = nn.BCEWithLogitsLoss(reduction="none")
    ...
    def forward(
        ...
        posterior_a_mask_logits = self.answer_decoder(c_ids, posterior_za)
        prior_a_mask_logits = self.answer_decoder(c_ids, prior_za)
    ...
    def compute_loss(self, out: Dict, batch: torch.Tensor, optimizer_idx):
        ...
        if optimizer_idx == 0:
            ...
            loss_a_rec = self.a_rec_criterion(posterior_a_mask_logits, a_mask.float()).sum(dim=-1).mean()
            ...
            D_a_real_pri_loss = self.lambda_gan_a * F.binary_cross_entropy_with_logits(
                D_a_real_pri, ones, reduction="none")
            ...
            D_a_fake_pri = self.a_discriminator(c_ids, prior_a_mask_logits.sigmoid())
